...
A single `backend/.env` (`DATABASE_URL=...`, `CALLMEBOT_PHONE=...`, `CALLMEBOT_APIKEY=...`) holds real credentials and is gitignored via `backend/.gitignore`. Both `datebase_connection.py` and `ws_notifier.py` load it via `Path(__file__).resolve().parents[3] / ".env"`, which resolves to `backend/.env` from either module's location. Never commit it, and never print its contents — if you need to check its shape, inspect structure only (e.g. `grep -oE '^[A-Z_]+=' backend/.env`), not values.
